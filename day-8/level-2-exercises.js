...
for (let i = 0; i < usersArray.length; i++){
  allKeys = Object.keys(users[usersArray[i]]);
}
...
for (let key in users){
  allValues = Object.values(users[key]);
  valuesArray.push(allValues);
}
...
let totalCountries = Object.keys(countries).length;
let totalKeys = Object.keys(countries);

randomCountry = Math.round(Math.random() * totalCountries)
let valuesCountry = Object.values(countries[totalKeys[randomCountry]]);

...
for (const country of countries){
  country.languages.forEach((language) => setLanguages.add(language));    
}
...
function mostSpokenLanguages(countries, number){
  ...
  for (const country of countries){
    country.languages.forEach(language => {
      if (mapLanguages.has(language)){
        mapLanguages.set(language, mapLanguages.get(language) + 1);
      }
      else{
        mapLanguages.set(language, 1);
      }
    });
  }
  const arrayLanguages = [...mapLanguages.entries()];
  arrayLanguages.sort((a,b) => b[1] - a[1]);

  const mostSpoken = arrayLanguages.slice(0,number)
